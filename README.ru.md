...
О каком именно `Arg` эти строки, решает резолвер, а не написанное имя: чужой
`Acme\Console\Arg::int()` внутри спецификации сохраняет свои диагностики, а наш
под алиасом теряет их под любым именем. По форме читается только `capture()` —
как вызов метода без аргументов, потому что тип получателя недоступен там, где
перехватывается репорт; поэтому чужой `capture()` без аргументов, написанный
внутри спецификации, будет принят за капторовский.
